...
public class RandomInstanceGenerator {
  ...
  private synchronized<T> T generate(Type type,
                                     Consumer<TypeGenerator> setUpGenerator)
   ...
   *                                     default constructor , class have a
   * non-public default constructor , setter cannot be invoked ... )
   */
   ...
   * non-public default constructor , setter cannot be invoked ... )
   */
  public <T> T generate(@NonNull Type type, int collectionSize)
      throws InstanceGenerationException {
   ...
   * non-public default constructor , setter cannot be invoked ... )
   */
  public <T> T generate(@NonNull Type objectType, int minSizeInclusive,
   ...
   * non-public default constructor , setter cannot be invoked ... )
   */
  public <T> T generate(@NonNull Type objectType, @NonNull Type... genericTypes)
      throws InstanceGenerationException {
    Type parameterizedType =
        new ParameterizedTypeImpl(genericTypes, (Class<?>)objectType);
  ...
  public <T> T generate(@NonNull Type type, int collectionSize,
      ...
      throws InstanceGenerationException {
    Type parameterizedType =
        new ParameterizedTypeImpl(genericTypes, (Class<?>)type);
    return generate(parameterizedType, collectionSize);
  }

  /**
   * generate a random instance of a generic collection with a size between a
   * range
   *
   * @param genericTypes types of generic parameters
   * @return the generated object
   ...
   * non-public default constructor , setter cannot be invoked ... )
   */
  public <T> T generate(@NonNull Type type, int minSizeInclusive,
                        int maxSizeExclusive, @NonNull Type... genericTypes)
      throws InstanceGenerationException {
    Type parameterizedType =
        new ParameterizedTypeImpl(genericTypes, (Class<?>)type);
   ...
   * because there is recursion in this object's fields (there is a field that
   * it's instantiation depends on owner object) so NestedObjectRecursion is
   * thrown
   *
  ...
  private void validateSize(int minSizeInclusive, int maxSizeExclusive) {
    Validate.isTrue(maxSizeExclusive > minSizeInclusive,
                    "Start value must be smaller than end value.");
    Validate.isTrue(minSizeInclusive >= 0,
