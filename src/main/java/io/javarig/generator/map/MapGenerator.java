...
public abstract class MapGenerator extends AbstractTypeGenerator implements TypeBasedGenerator, CollectionGenerator {
    ...
    private void checkIfValidNumberOfGenericArguments(ParameterizedType type) {
        if (type.getActualTypeArguments().length != NUMBER_OF_GENERIC_PARAMS) {
            throw new InvalidGenericArgumentsNumberException(NUMBER_OF_GENERIC_PARAMS, type.getActualTypeArguments().length);
        }
    }
